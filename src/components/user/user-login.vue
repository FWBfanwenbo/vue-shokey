        ...
        <section class="reg-center">
            <div class="form-wrapper inner">
                ...
                <form action="" id="reg-form">
                    ...
                    <input class="reg-submit" type="button" @click="EnterUser" value="登录">
                </form>
            </div>
            <keep-alive>
        ...
        methods:{
            checkphone:function(){
                ...
                if(this.phone==""){
                    ...
                    this.$post('/api/FwbServer.ashx?command=UserCheckLoginPhone',
                    JSON.stringify({
                        phone:that.phone
                    })
                    ).then((res)=>{
            ...
            EnterUser(){
                var that=this;
                this.$post("/api/FwbServer.ashx?command=UserCheckLogin",
                    JSON.stringify({
                        phone:that.phone,
                        pass:that.pass
                    })
                    ...
                    if(res.data[0].msg=="ok"){
                        sessionStorage.user_phone=res.data[0].phone;
                        this.$router.push({
                            path:"/user-center"
                        })
                    }else{
                        console.log("密码或手机号码有误")
                    }
                })
